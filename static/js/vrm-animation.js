...
class VRMAnimation {
    ...
    constructor(manager) {
        ...
        // 分层动画控制器
        this.vrmaMixer = null;            // 负责肢体和身体动作 (Layer 0)
        this.vrmaExpressionMixer = null;  // 负责表情与嘴唇 (Layer 1)
    ...
    update(delta) {
        ...
        if (this.vrmaIsPlaying) {
            // 推进身体层
            if (this.vrmaMixer) this.vrmaMixer.update(updateDelta);
            // 推进表情层
            if (this.vrmaExpressionMixer) this.vrmaExpressionMixer.update(updateDelta);
            ...
            if (vrm?.scene) {
                ...
                if (vrm.humanoid) {
                    ...
                    if (vrmVersion === '1.0' && vrm.humanoid.autoUpdateHumanBones) {
                        ...
                        const mixerRoot = this.vrmaMixer ? this.vrmaMixer.getRoot() : vrm.scene;
    ...
    _createAndConfigureAction(clip, mixerRoot, options, vrm) {
        if (this.vrmaMixer) {
            this.vrmaMixer.stopAllAction();
        ...
        if (this.vrmaExpressionMixer) {
            this.vrmaExpressionMixer.stopAllAction();
            const exprRoot = this.vrmaExpressionMixer.getRoot();
            if (exprRoot) this.vrmaExpressionMixer.uncacheRoot(exprRoot);
            this.vrmaExpressionMixer = null;
        }
        this.currentAction = null;
        this.currentExpressionAction = null;
        this.vrmaIsPlaying = false;

        // 【核心分离】将动画拆分为 身体层 和 表情层 (BlendShapes)
        const bodyTracks = [];
        const expressionTracks = [];

        clip.tracks.forEach(track => {
            // Expression 轨道在 three-vrm 中常以 name 结尾为 'expression' 等标记
            // 具体名称由 VRMAnimationLoaderPlugin 生成，或者看它是不是操纵 blendshape
            if (track.name.toLowerCase().includes('expression') || track.name.toLowerCase().includes('blendshape')) {
                expressionTracks.push(track);
            } else {
        ...
        // 重组分离的 Clip
        const bodyClip = new window.THREE.AnimationClip(`${clip.name}_body`, clip.duration, bodyTracks);
        const expressionClip = expressionTracks.length > 0
            ? new window.THREE.AnimationClip(`${clip.name}_expression`, clip.duration, expressionTracks)
            : null;

        // 1. 设置身体 Mixer
        this.vrmaMixer = new window.THREE.AnimationMixer(mixerRoot);
        const bodyAction = this.vrmaMixer.clipAction(bodyClip);
        if (!bodyAction) {
            throw new Error(window.t ? window.t('vrm.error.cannotCreateAnimationAction') : '无法创建身体动画动作');
        }

        bodyAction.enabled = true;
        bodyAction.setLoop(options.loop ? window.THREE.LoopRepeat : window.THREE.LoopOnce);
        bodyAction.clampWhenFinished = true;

        // 2. 设置表情 Mixer（挂载到 VRM 实例或其表情管理器上）
        // 对于 @pixiv/three-vrm 的表情轨道，其绑定节点通常为 vrm.expressionManager 或者是 scene
        let exprAction = null;
        if (expressionClip) {
            // 表情混合器的 root 一般还是 vrm.scene 即可，因为内部绑定机制会处理
            this.vrmaExpressionMixer = new window.THREE.AnimationMixer(vrm.scene);
            exprAction = this.vrmaExpressionMixer.clipAction(expressionClip);
            if (exprAction) {
                exprAction.enabled = true;
                // 表情动画跟随主循环设置
                exprAction.setLoop(options.loop ? window.THREE.LoopRepeat : window.THREE.LoopOnce);
                exprAction.clampWhenFinished = true;
            }
        }

        this.playbackSpeed = (options.timeScale !== undefined) ? options.timeScale : 1.0;
        bodyAction.timeScale = 1.0;
        if (exprAction) exprAction.timeScale = 1.0;

        return { bodyAction, exprAction };
    }

    _playAction(actions, options, vrm) {
        if (!this.vrmaMixer) {
            console.error('[VRM Animation] _playAction: vrmaMixer 未初始化');
            return;
        }

        const { bodyAction: newBodyAction, exprAction: newExprAction } = actions;
        const fadeDuration = options.fadeDuration !== undefined ? options.fadeDuration : 0.4;
        const isImmediate = options.immediate === true;

        const _play = (oldAction, newAction, mixer) => {
            if (!newAction) return;
            if (isImmediate) {
            ...
            if (newAction.paused) newAction.play();
        };

        _play(this.currentAction, newBodyAction, this.vrmaMixer);
        if (this.vrmaExpressionMixer && newExprAction) {
            _play(this.currentExpressionAction, newExprAction, this.vrmaExpressionMixer);
        }

        this.currentAction = newBodyAction;
        this.currentExpressionAction = newExprAction || null;
        this.vrmaIsPlaying = true;

        if (this.vrmaMixer) this.vrmaMixer.update(0.001);
        if (this.vrmaExpressionMixer) this.vrmaExpressionMixer.update(0.001);
    ...
    async playVRMAAnimation(vrmaPath, options = {}) {
        ...
        try {
            ...
            const actions = this._createAndConfigureAction(clip, mixerRoot, options, vrm);
            this._playAction(actions, options, vrm);

            // 记录最近一次播放的动画 URL，供切换模型后重播使用
            this._lastVrmaUrl = vrmaPath;
    ...
    stopVRMAAnimation() {
        ...
        if (this.currentAction) {
            ...
            if (this.currentExpressionAction) this.currentExpressionAction.fadeOut(0.5);
            ...
            this._fadeTimer = setTimeout(() => {
                ...
                if (this.currentAction === actionAtStop) {
                    ...
                    if (this.vrmaExpressionMixer) this.vrmaExpressionMixer.stopAllAction();
    ...
    _updateLipSync(delta) {
        ...
        try {
            // **重构点**: 提取 Layer 口型权重避免与 VRMA_Expression 相乘导致动画过载。
            // 只有当有声音输入的时候才会施加口型张合（如果正在播放其他表情，系统会通过累加叠加）。
    ...
    reset() {
        ...
        if (this.vrmaExpressionMixer) {
            this.vrmaExpressionMixer.stopAllAction();
            const root = this.vrmaExpressionMixer.getRoot();
            if (root) this.vrmaExpressionMixer.uncacheRoot(root);
            this.vrmaExpressionMixer = null;
        }
