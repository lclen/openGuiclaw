...
class VRMInteraction {
    constructor(manager) {
        ...
        // 开启"始终面朝相机" (已禁用，允许用户完整旋转模型查看背面)
        this.enableFaceCamera = false;
        ...
        // 性能优化：预分配可复用的向量，避免拖拽/旋转时每帧创建新对象触发 GC
        this._v3_a = THREE ? new THREE.Vector3() : null;
        this._v3_b = THREE ? new THREE.Vector3() : null;
        this._v3_c = THREE ? new THREE.Vector3() : null;
        this._v3_right = THREE ? new THREE.Vector3() : null;
        this._v3_up = THREE ? new THREE.Vector3() : null;
        this._v3_newPos = THREE ? new THREE.Vector3() : null;
        this._zoomDirection = THREE ? new THREE.Vector3() : null;
    }
     ...
     * @param {boolean} precise - 是否执行精确的三维射线检测（逐三角面），默认为 true
     * @returns {boolean} 是否命中
     */
    _hitTestModel(clientX, clientY, precise = true) {
        if (!this.manager.currentModel?.scene) return false;

        // 1. 性能优化：优先使用缓存的屏幕包围盒进行快速过滤
        if (!this._cachedScreenBounds) {
            this.updateModelBoundsCache();
        }

        if (this._cachedScreenBounds) {
            const padding = 10; // 适当放宽边缘，提升手感
        ...
        // 如果不需要精确检测（如仅用于悬停时光标切换），包围盒命中即视为成功
        if (!precise) return true;

        // 2. 精确射线检测（常用于点击判定）
        if (!this._raycaster || !this.manager.camera) {
            return false;
        }
        ...
        // 注意：VRM 模型通常包含大量网格，recursive: true 是必要的但开销大
        // 通过之前的包围盒过滤，我们已经避免了大部分无效计算
    ...
    initDragAndZoom() {
        ...
        this.mouseDownHandler = (e) => {
            ...
            } else if (e.button === 0) { // 左键 - 相机视角旋转
                this.isDragging = true;
                this.dragMode = 'orbit';
                this.previousMousePosition = { x: e.clientX, y: e.clientY };
                canvas.style.cursor = 'crosshair';
                // 移除 e.preventDefault() 避免偶尔干扰原生点击事件
                e.stopPropagation();

                // 记录模型中心和它在屏幕上的 NDC 坐标，旋转过程中用于保持模型屏幕位置不变
                if (this.manager.camera && this.manager.currentModel?.scene) {
                    // 性能优化：使用缓存的 localBox 代替 O(N) 的 setFromObject
                    if (!this.manager.currentModel.scene.userData.localBox) {
                        this.manager.currentModel.scene.userData.localBox = new THREE.Box3().setFromObject(this.manager.currentModel.scene);
                    }
                    if (!this._orbitCenter) this._orbitCenter = new THREE.Vector3();
                    this.manager.currentModel.scene.userData.localBox.getCenter(this._orbitCenter);
                    // 应用世界矩阵偏移（localBox 是局部坐标，需要加上场景偏移）
                    this._orbitCenter.add(this.manager.currentModel.scene.position);
                    // 将模型中心投影到 NDC 坐标（-1~1），记录模型在屏幕上的位置
                    if (!this._v3_a) this._v3_a = new THREE.Vector3();
                    this._v3_a.copy(this._orbitCenter).project(this.manager.camera);
                    this._orbitNDC = { x: this._v3_a.x, y: this._v3_a.y };
                }
        ...
        this.dragHandler = (e) => {
            ...
            if (this.dragMode === 'pan' && this.manager.currentModel && this.manager.currentModel.scene) {
                // 性能优化：所有 Vector3 使用预分配的复用对象，避免每帧 GC 压力
                const camera = this.manager.camera;
                const renderer = this.manager.renderer;

                // 计算相机到模型中心的距离（复用 _v3_a 存模型中心位置）
                const modelCenter = this.manager.currentModel.scene.position;
                ...
                // 复用预分配向量，避免每帧 new THREE.Vector3()
                this._v3_right.set(1, 0, 0).applyQuaternion(camera.quaternion);
                this._v3_up.set(0, 1, 0).applyQuaternion(camera.quaternion);

                // 计算新位置（复用 _v3_newPos）
                this._v3_newPos.copy(modelCenter);
                this._v3_newPos.add(this._v3_right.multiplyScalar(deltaX * pixelToWorldX));
                this._v3_newPos.add(this._v3_up.multiplyScalar(-deltaY * pixelToWorldY));

                const finalPosition = this.clampModelPosition(this._v3_newPos);
                ...
                // 性能优化: 复用 _v3_a 存放 offset（相机 → 旋转中心），避免 .clone()
                this._v3_a.subVectors(camera.position, orbitCenter);
                const radius = this._v3_a.length();

                let theta = Math.atan2(this._v3_a.x, this._v3_a.z);
                let phi = Math.acos(Math.max(-1, Math.min(1, this._v3_a.y / radius)));

                theta -= deltaX * orbitSpeed;
                phi -= deltaY * orbitSpeed;
                phi = Math.max(0.087, Math.min(Math.PI - 0.087, phi));

                // 直接修改 _v3_a（即 offset），无需 new Vector3
                this._v3_a.set(
                ...
                camera.position.copy(orbitCenter).add(this._v3_a);
                ...
                // 复用 _v3_right / _v3_up，避免每帧 new THREE.Vector3()
                this._v3_right.set(1, 0, 0).applyQuaternion(camera.quaternion);
                this._v3_up.set(0, 1, 0).applyQuaternion(camera.quaternion);

                // 复用 _v3_newPos 作为 newTarget，避免 .clone() 和 .sub()
                this._v3_newPos.copy(orbitCenter)
                    .sub(this._v3_right.multiplyScalar(nx * halfWidth))
                    .sub(this._v3_up.multiplyScalar(ny * halfHeight));

                camera.lookAt(this._v3_newPos);
                if (!this.manager._cameraTarget) this.manager._cameraTarget = new THREE.Vector3();
                this.manager._cameraTarget.copy(this._v3_newPos);
            }
        ...
        this.mouseHoverHandler = (e) => {
            ...
            _hoverThrottleId = requestAnimationFrame(() => {
                ...
                // 性能优化：悬停检测无需精确的面片级计算，包围盒判定即可满足需求，极大降低 CPU 开销
                if (this._hitTestModel(e.clientX, e.clientY, false)) {
        ...
        this.wheelHandler = (e) => {
            ...
            if (this.manager.currentModel.scene && this.manager.camera) {
                ...
                // 复用静态向量，避免每次滚轮创建新对象造成 GC 压力
                if (!this._zoomDirection) this._zoomDirection = new THREE.Vector3();
                this._zoomDirection.subVectors(this.manager.camera.position, zoomCenter).normalize();

                this.manager.camera.position.copy(zoomCenter)
                    .add(this._zoomDirection.multiplyScalar(newDistance));

                if (this.manager.controls && this.manager.controls.update) {
                    this.manager.controls.update();
                }

                // 【已禁用】缩放结束后防抖保存位置，改为手动保存
    ...
    clampModelPosition(position) {
        ...
        try {
            // 性能优化: 使用缓存局部盒子 + 矩阵偏移，避免 O(N) 的 setFromObject 遍历
            // 通过目标位置与当前位置的差量偏移局部盒子，无需实际移动模型
            if (!vrm.scene.userData.localBox) {
                vrm.scene.userData.localBox = new THREE.Box3().setFromObject(vrm.scene);
            }

            const offset = position.clone().sub(vrm.scene.position);
            const box = vrm.scene.userData.localBox.clone().translate(offset).applyMatrix4(vrm.scene.matrixWorld);
            ...
            // UI 占据左侧 75% (0.75 * screenWidth)，模型必须在屏幕右侧可见。
            const leftBoundary = screenWidth * 0.75;

            // 如果模型完全进入左侧 UI 区域底下
            if (modelMaxX < leftBoundary + MIN_VISIBLE_PIXELS) {
                moveX = (leftBoundary + MIN_VISIBLE_PIXELS) - modelMaxX;
            }
            // 如果模型完全在屏幕右侧外
    ...
    updateModelBoundsCache() {
        ...
        try {
            ...
            // 性能优化: 高面数模型频繁调用 setFromObject (O(N) 遍历所有顶点) 会导致严重的 CPU 卡顿
            // 我们只在第一次计算局部包围盒，后续仅应用世界矩阵。
            if (!vrm.scene.userData.localBox) {
                vrm.scene.userData.localBox = new THREE.Box3().setFromObject(vrm.scene);
            }

            // 使用缓存的局部包围盒，结合当前的世界矩阵进行快速变换
            this._cachedBox = vrm.scene.userData.localBox.clone().applyMatrix4(vrm.scene.matrixWorld);
